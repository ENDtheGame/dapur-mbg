<template>
  <div v-if="isOpen" @click="$emit('close')" class="fixed inset-0 bg-black/20 z-40 md:hidden"></div>

  <aside :class="['fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-slate-100 transform transition-transform duration-300 md:relative md:translate-x-0', isOpen ? 'translate-x-0' : '-translate-x-full']">
    ...
    <div class="p-6 flex items-center gap-3">
      ...
      <div>
        ...
        <p class="text-[10px] font-semibold text-sky-600 tracking-wider uppercase mt-0.5">
    ...
    <nav class="mt-4 px-4 space-y-1">
      <router-link 
        ...
        :class="$route.path === item.path ? 'bg-sky-50 text-sky-700' : 'text-slate-500 hover:bg-slate-50'"
      >
...
import { LayoutDashboard, Users, Calendars, ShelvingUnit,  Settings, LogOut, DownloadIcon, FileSliders } from 'lucide-vue-next';
