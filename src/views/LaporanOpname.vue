<template>
  <div class="p-6 bg-slate-50 min-h-screen pb-24">
    <div class="mb-8">
      <div
        class="mt-4 bg-white p-4 rounded-[2rem] border border-slate-100 shadow-sm space-y-4"
      >
    ...
    <div class="grid grid-cols-2 gap-4 mb-8">
      <div
        class="bg-emerald-500 p-5 rounded-[2.5rem] text-white shadow-lg shadow-emerald-100"
      >
        <TrendingUp :size="20" class="mb-2 opacity-80" />
        <div class="text-2xl font-black">{{ totalMasukRef }}</div>
        <div class="text-[9px] font-bold uppercase tracking-widest opacity-80">
          Total Barang Masuk
        </div>
      </div>
      <div
        class="bg-rose-500 p-5 rounded-[2.5rem] text-white shadow-lg shadow-rose-100"
      >
    ...
    <div
      class="bg-white rounded-[2.5rem] border border-slate-100 overflow-hidden shadow-sm mb-10"
    >
      <div
      ...
      <div class="divide-y divide-slate-50">
        <div
          v-if="loading"
          class="p-10 text-center text-slate-300 text-xs italic animate-pulse"
        >
          Memuat data...
        </div>
        <div
          v-else-if="logs.length === 0"
          class="p-10 text-center text-slate-300 text-xs italic"
        >
        ...
        >
          <div class="flex items-center gap-3">
            ...
            <div
              :class="
                log.tipe_perubahan?.toUpperCase() === 'MASUK'
            ...
            >
              <Plus
                v-if="log.tipe_perubahan?.toUpperCase() === 'MASUK'"
                :size="16"
              />
            ...
            <div>
              <div class="flex items-center gap-2">
                <h4 class="font-bold text-slate-700 text-xs">
                  {{ log.stok_bahan?.nama_bahan }}
                </h4>
                <span
                  class="px-1.5 py-0.5 rounded text-[7px] font-black uppercase tracking-tighter"
                  :class="
                    log.stok_bahan?.kategori?.tipe === 'ASET'
          ...
          <div class="text-right">
            ...
            >
              ...
              <span class="text-[8px] font-bold opacity-60 ml-0.5">{{
                log.stok_bahan?.satuan_kecil?.nama_satuan
              }}</span>
            </div>
            <p
              v-if="log.keterangan"
              class="text-[8px] text-slate-400 font-medium italic mt-0.5"
            >
              "{{ log.keterangan }}"
            </p>
          </div>
        ...
        <div class="flex items-center justify-between p-6">
          ...
          <div class="text-[10px] font-black text-slate-400 uppercase">
            Hal. {{ currentPage + 1 }} /
            {{ Math.ceil(totalData / itemsPerPage) || 1 }}
          </div>
          <button
            @click="fetchData(currentPage + 1)"
...
const hasNextPage = computed(
  () => (currentPage.value + 1) * itemsPerPage < totalData.value,
);
...
const fetchData = async (page = 0) => {
  ...
  try {
    ...
    let query = supabase.from("riwayat_gudang").select(
      `
        *,
        stok_bahan!inner (
          nama_bahan,
    ...
    let totalQ = supabase.from("riwayat_gudang").select(`
        jumlah_perubahan, 
        tipe_perubahan,
        stok_bahan!inner (
...
const exportKeExcel = async () => {
  ...
  title.value = "SPPG Garut Cigedug Cigedug";
  ...
  subtitle.value =
    "Desa Cigedug, Kecamatan Cigedug, Kabupaten Garut, Jawa Barat";
  ...
  worksheet.getCell(`F${lastR + 2}`).value = `Garut, ${tglCetak}`;
  ...
  saveAs(
    new Blob([buffer]),
    `Laporan_Gudang_SPPG_CIGEDUG_${new Date().getTime()}.xlsx`,
  );
};
